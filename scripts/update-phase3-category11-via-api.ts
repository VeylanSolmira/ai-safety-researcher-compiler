...
// Script to update Phase 3 Category 11 content via API

const topics = [
  {
    id: 'mechanistic-interp',
    contentAcademic: `# Mechanistic Interpretability Practice
...
- **Apollo Research**: Investigating deceptive AI through interpretability`,
    contentPersonal: `# Real Talk: Mechanistic Interpretability
...
Welcome to the dig site. Grab a shovel.`
  ...
  {
    id: 'explainable-ai',
    contentAcademic: `# Building Explainable AI Systems
...
- **Autonomous Vehicles**: Explaining driving decisions`,
    contentPersonal: `# Building AI That Can Actually Explain Itself
...
Remember: An unexplainable AI that works is often more dangerous than an explainable AI that's slightly worse. Choose wisely.`
  ...
  {
    id: 'debugging-tools',
    contentAcademic: `# AI Debugging Frameworks
...
- **Incident Response**: Rapid debugging when things go wrong`,
    contentPersonal: `# AI Debugging: The Reality Check
...
async function updateTopic(topic: { id: string; contentAcademic: string; contentPersonal: string }) {
  const baseUrl = 'http://localhost:3002'
  
  try {
    const response = await fetch(`${baseUrl}/api/topics/${topic.id}`, {
      method: 'PATCH',
      headers: {
        'Content-Type': 'application/json',
      },
      body: JSON.stringify({
        content_academic: topic.contentAcademic,
        content_personal: topic.contentPersonal,
      }),
    })

    if (!response.ok) {
      const error = await response.text()
      throw new Error(`Failed to update ${topic.id}: ${response.status} - ${error}`)
    }
...
async function main() {
  console.log('Updating Phase 3 Category 11 content via API...')

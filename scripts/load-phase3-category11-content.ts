...
function main() {
  console.log('Loading Phase 3 Category 11 content into database...')
  
  for (const content of interpretabilityContent) {
    try {
      // Check if topic exists
      const existing = db.select().from(topics).where(eq(topics.id, content.slug)).get()
      
      if (!existing) {
        console.log(`Topic ${content.slug} not found in database. Skipping...`)
        continue
      }
      
      // Update the topic with content
      try {
        const result = db.update(topics)
          .set({
            content_academic: content.content_academic,
            content_personal: content.content_personal
          })
          .where(eq(topics.id, content.slug))
          .run()
      } catch (updateError) {
        console.error(`SQL Error for ${content.slug}:`, updateError)
        // Try with different field names
        try {
          const result2 = db.update(topics)
            .set({
              contentAcademic: content.content_academic,
              contentPersonal: content.content_personal
            })
            .where(eq(topics.id, content.slug))
            .run()
          console.log(`✅ Updated content for ${content.slug} (using camelCase fields)`)
          continue
        } catch (err2) {
          console.error(`Failed with camelCase too:`, err2)
        }
      }
      
      console.log(`✅ Updated content for ${content.slug}`)
    } catch (error) {
      console.error(`Error updating ${content.slug}:`, error)
    }
  }
  
  console.log('Done! All Phase 3 Category 11 content has been loaded into the database.')
  process.exit(0)
}
